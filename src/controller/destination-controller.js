...
const update = async (req, res, next) => {
    try {
        ...
        // Log untuk debugging
        console.log(`Memproses update untuk destinasi ${destinationId} di kategori ${categoryId}`);
        console.log("Request body:", JSON.stringify(req.body, null, 2));
        console.log("Request files:", req.files ? Object.keys(req.files).join(', ') : "No files");
        ...
        // Proses removedPictureIds jika ada
        if (req.body.removedPictureIds) {
            // Jangan mencoba mem-parse di controller, biarkan service menanganinya
            // Kirim nilai removedPictureIds apa adanya
            requestData.removedPictureIds = req.body.removedPictureIds;
            console.log("removedPictureIds dari request:", requestData.removedPictureIds);
        }
        ...
        if (req.files) {
            ...
            if (req.files.picture && req.files.picture.length > 0) {
                console.log(`Processing ${req.files.picture.length} picture images...`);
        ...
        console.log("Memanggil service update...");
        const result = await destinationService.update(categoryId, requestData);
        console.log("Update berhasil, mengembalikan response");

        res.status(200).json({
            data: sanitizeResponse(result)
        });
    } catch (e) {
        console.error("Error di controller update:", e);
        next(e);
    }
};
